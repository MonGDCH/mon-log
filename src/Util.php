...
class Util
{
     ...
     * @return boolean
     */
    public static function createFile(string $content, string $path, $append = true): bool
    {
     ...
     * @return boolean
     */
    public static function subsectionFile(string $content, string $path, int $maxSize = 20480000, int $rollNum = 3, string $postfix = '.log'): bool
    {
        // 日志路径
        ...
        if (file_exists($destination) && floor($maxSize) < (filesize($destination) + $contentLength)) {
            // 超出剩余写入大小，分卷写入
            static::shiftFile($path, $rollNum, $postfix);
            return static::createFile($content, $destination, false);
        }

        // 不存在文件或文件大小足够继续写入
        return static::createFile($content, $destination);
    }
     ...
     * @return void
     */
    protected static function shiftFile(string $path, int $rollNum, string $postfix = '.log')
    {
        // 判断是否存在最老的一份文件，存在则删除
        $oldest = static::buildShiftName($path, ($rollNum - 1));
        ...
        for ($i = ($rollNum - 2); $i >= 0; $i--) {
            ...
            if ($i == 0) {
                ...
                // 获取分卷号文件名称
                $oldFile = static::buildShiftName($path, $i);
            }

            // 重命名文件
            $oldFileName = $oldFile . $postfix;
            if (file_exists($oldFileName)) {
                $newFileNmae = static::buildShiftName($path, ($i + 1)) . $postfix;
                // 重命名
     ...
     * @return string
     */
    protected static function buildShiftName(string $fileName, int $num): string
    {
